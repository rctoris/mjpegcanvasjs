...
MJPEGCANVAS.Canvas = function(options) {
  var that = this;
  options = options || {};
  ...
  this.showMenus = options.showMenus;
  ...
  var createImage = function() {
    ...
    img = new Image();
    var src = 'http://' + that.host + ':' + that.port + '/stream?topic=' + curStream;
  ...
  if (this.topic instanceof Array) {
    // check the labels
    if (!(this.label && this.label instanceof Array && this.label.length === this.topic.length)) {
      this.label = null;
    }

    // start the stream with the first topic
    curStream = this.topic[this.defaultStream];
        ...
        .addEventListener(
            ...
            function(e) {
              ...
              if (x < (buttonWidth + buttonMargin) && x > buttonMargin
                  && y > (height - (buttonHeight + buttonMargin)) && y < height - buttonMargin) {
                menuOpen = true;

                // create the menu
                var html = '<h2>Camera Streams</h2><hr /><br /><form><label for="stream">Stream</label><select name="stream" id="stream-'
                    + id + '" required>';
                for ( var i = 0; i < that.topic.length; i++) {
                  // check if this is the selected option
                  var selected = '';
                  if (that.topic[i] === curStream) {
                    selected = 'selected="selected"';
                  }
                  html += '<option value="' + that.topic[i] + '" ' + selected + '>';
                  // check for a label
                  if (that.label) {
                    html += that.label[i];
                  } else {
                ...
                html += '</select></form><br /><button id="button-' + id + '">Close</button>';
  ...
  function draw(img) {
    ...
    if (img) {
      ...
      if (img.width > 0 && img.height > 0) {
        ...
        context.drawImage(errorIcon, (width - (width / 2)) / 2, (height - (height / 2)) / 2,
            width / 2, height / 2);
        errorDrawn = true;
        this.emit('error', 'Invalid stream');
      }
    } else {
      context.clearRect(0, 0, width, height);
    }

    if ((mouseEnter || this.showMenus) && !menuOpen) {
      // create the "swap" button
      context.globalAlpha = 0.66;
      context.beginPath();
      context.rect(buttonMargin, height - (buttonHeight + buttonMargin), buttonWidth, buttonHeight);
  ...
  // redraw the image every 30 ms
  this.drawInterval = setInterval(function() {
    draw(img);
  }, 30);
};
MJPEGCANVAS.Canvas.prototype.__proto__ = EventEmitter2.prototype;

MJPEGCANVAS.Canvas.prototype.destroy = function() {
